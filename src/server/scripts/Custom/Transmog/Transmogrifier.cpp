...
namespace
{
    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
			...
			AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Toggle Transmog Display", EQUIPMENT_SLOT_END + 11, 0);
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
        {
            ...
            switch (sender)
            {
				...
				{
					if (player->CanSeeTransmog())
					{
						player->SetCanSeeTransmog(false);
						ChatHandler(player->GetSession()).SendSysMessage("Transmog Display Disabled!");
					}
					else
					{
						player->SetCanSeeTransmog(true);
						ChatHandler(player->GetSession()).SendSysMessage("Transmog Display Enabled!");
					}						
				} break;
