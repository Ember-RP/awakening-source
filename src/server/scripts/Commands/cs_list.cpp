...
public:
    ...
    static bool HandleListAurasCommand(ChatHandler* handler, char const* /*args*/)
    {
		...
		handler->PSendSysMessage(LANG_COMMAND_TARGET_LISTAURAS, unit->GetAppliedAuras().size());
		
        for (auto const &kvPair : unit->GetAppliedAuras())
        {
            AuraApplication const* aurApp = kvPair.second;
