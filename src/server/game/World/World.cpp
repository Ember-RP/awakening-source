...
void World::TriggerGuidWarning()
{
	...
	while (GameTime::GetGameTime() >= (today + (m_int_configs[CONFIG_RESPAWN_RESTARTQUIETTIME]) * HOUR) - 1810)
		today += DAY;
...
void World::LoadConfigSettings(bool reload)
{
	...
	m_int_configs[CONFIG_WHO_COUNT] = sConfigMgr->GetIntDefault("WhoCount", 100);
	m_int_configs[CONFIG_WHO_COUNT_MULTIPLIER] = sConfigMgr->GetIntDefault("WhoCountMultiplier", 1);
...
void World::Update(uint32 diff)
{
	...
	{
		warnDiff += diff;
		if (currentGameTime >= warnShutdownTime)
