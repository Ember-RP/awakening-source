...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
	...
	//AWAKE: Check the query to see if there's a summoned pet
	if (fields[16].GetUInt8() == SUMMON_PET && petEntry == 0)
		return false;
    ...
    switch (getPetType())
    {
        case SUMMON_PET:
            ...
            break;
        case HUNTER_PET:
			SetSummonedBySpellId(883); // Awake
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
	...
	if (IsPet() && GetOwner()->GetTypeId() == TYPEID_PLAYER) // Awake
	{
		if (ToPet()->GetEntry() == ENTRY_IMP || ToPet()->GetEntry() == ENTRY_VOIDWALKER || ToPet()->GetEntry() == ENTRY_SUCCUBUS || ToPet()->GetEntry() == ENTRY_FELHUNTER
			|| ToPet()->GetEntry() == ENTRY_FELGUARD || ToPet()->GetEntry() == ENTRY_WATER_ELEMENTAL || ToPet()->GetEntry() == ENTRY_TREANT || ToPet()->GetEntry() == ENTRY_FIRE_ELEMENTAL 
			|| ToPet()->GetEntry() == ENTRY_GHOUL || ToPet()->GetEntry() == ENTRY_BLOODWORM || ToPet()->GetEntry() == ENTRY_SPIRITWOLF || ToPet()->GetEntry() == ENTRY_EARTH_ELEMENTAL)
		{
			petType = SUMMON_PET;
		}
		else
