...
bool Player::BuildEnumData(PreparedQueryResult result, WorldPacket* data)
{
    ...
    *data << uint8(atLoginFlags & AT_LOGIN_FIRST ? 1 : 0);
	...
	if (result && !(playerFlags & PLAYER_FLAGS_GHOST) && (plrClass == CLASS_WARLOCK || plrClass == CLASS_DRUID || plrClass == CLASS_DEATH_KNIGHT)) //Awakening HUNTER TO DRUID
    {
...
void Player::DuelComplete(DuelCompleteType type)
{
    ...
    /* remove auras */
    auto &itsAuras = duel->opponent->GetAppliedAuras();
    for (auto i = itsAuras.begin(); i != itsAuras.end();)
    {
    ...
    auto &myAuras = GetAppliedAuras();
    for (auto i = myAuras.begin(); i != myAuras.end();)
    {
        Aura const* aura = i->second->GetBase();
...
//                break;
    ...
    //AWAKENING:
	//Pass the slot and apply to handle single-gear armor application.	
	_RecalculateBaseArmor(slot, apply);
...
void Player::ApplyEquipSpell(SpellInfo const* spellInfo, Item* item, bool apply, bool form_change)
{
    if (apply)
    {
        ...
        {
             auto range = m_appliedAuras.equal_range(spellInfo->Id);
             for (; range.first != range.second; ++range.first)
                if (!item || range.first->second->GetBase()->GetCastItemGUID() == item->GetGUID())
                    return;
        }
...
//AWAKENING:
void Player::_RecalculateBaseArmor(uint8 pSlot, bool apply)
{
	// Handle armor class specific multipliers
	float armor_cloth_multiplier = 1.0f;
	float armor_leather_multiplier = 1.0f;
	float armor_mail_multiplier = 1.0f;
	float armor_plate_multiplier = 1.0f;

	float armorMod = 1;

	// Spells that only affect specific types of armor go here
	uint32 const DIRE_BEAR_FORM_AURA = 9635;
	uint32 const BEAR_FORM_AURA = 1178;
	uint32 const MOONKIN_FORM_AURA = 24905;
	uint32 const IMPROVED_BARKSKIN_1 = 63410;
	uint32 const IMPROVED_BARKSKIN_2 = 63411;
	uint32 const IMPROVED_TREE_OF_LIFE_1 = 48535;
	uint32 const IMPROVED_TREE_OF_LIFE_2 = 48536;
	uint32 const IMPROVED_TREE_OF_LIFE_3 = 48537;
	...
	for (AuraEffectList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
	{
		...
		{
		case DIRE_BEAR_FORM_AURA:
		case BEAR_FORM_AURA:
		case MOONKIN_FORM_AURA:
			armorMod = (*itr)->GetAmount() / 100.0f;
			armor_cloth_multiplier += armorMod;
			armor_leather_multiplier += armorMod;
			break;
		case IMPROVED_BARKSKIN_1:
		case IMPROVED_BARKSKIN_2:
			//Barkskin isn't active during shapeshifts
			...
			break;
		case IMPROVED_TREE_OF_LIFE_1:
		case IMPROVED_TREE_OF_LIFE_2:
		case IMPROVED_TREE_OF_LIFE_3:
			switch (GetShapeshiftForm()) 
			{
			case FORM_TREE:
			case FORM_NONE:
	...
	//If RecalculateBaseArmor was passed arguments other than default,
	//Calculate only the one item.
	if (pSlot != INVENTORY_SLOT_BAG_END)
	{
		if (m_items[pSlot])
		{
			// Don't apply armor bonuses for broken items
			if (m_items[pSlot]->IsBroken())
				return;

			// Check if the item exists
			ItemTemplate const* proto = m_items[pSlot]->GetTemplate();
			if (!proto)
				return;

			TC_LOG_DEBUG("entities.player", "RecalculateBaseArmor is processing %s", proto->Name1);
			...
			if (armor)
			{
				...
				if (proto->Class == ITEM_CLASS_ARMOR)
				{
					...
					case ITEM_SUBCLASS_ARMOR_CLOTH:
						modType = BASE_VALUE;
						armor *= armor_cloth_multiplier;
						break;
					case ITEM_SUBCLASS_ARMOR_LEATHER:
						modType = BASE_VALUE;
						armor *= armor_leather_multiplier;
						break;
					case ITEM_SUBCLASS_ARMOR_MAIL:
						modType = BASE_VALUE;
						armor *= armor_mail_multiplier;
						break;
					case ITEM_SUBCLASS_ARMOR_PLATE:
						modType = BASE_VALUE;
						armor *= armor_plate_multiplier;
						break;
				...
				if (apply)
				{
					m_auraFlatModifiersGroup[UNIT_MOD_ARMOR][BASE_VALUE] += armor;
				}
				else
				{
					m_auraFlatModifiersGroup[UNIT_MOD_ARMOR][BASE_VALUE] -= armor;
	...
	{
		m_auraFlatModifiersGroup[UNIT_MOD_ARMOR][BASE_VALUE] = 0;
		for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
		{
			if (m_items[i])
			{
...
uint8 Player::FindEquipSlot(ItemTemplate const* proto, uint32 slot, bool swap) const
{
    ...
    switch (proto->InventoryType)
    {
        ...
        case INVTYPE_RELIC:
        {
            switch (proto->SubClass)
            {
				case ITEM_SUBCLASS_ARMOR_LIBRAM:  //Awakening Ranged Slot
        ...
        if (Creature* creature = source->ToCreature())
        {
            ...
            switch (itr->second.OptionType)
            {
                ...
                case GOSSIP_OPTION_UNLEARNPETTALENTS:
                    if (!GetPet() || GetPet()->getPetType() != HUNTER_PET || GetPet()->m_spells.size() <= 1 || creature->GetCreatureTemplate()->trainer_type != TRAINER_TYPE_PETS || creature->GetCreatureTemplate()->trainer_class != CLASS_DRUID) //Awake 
                        canTalk = false;
                    break;
                ...
                case GOSSIP_OPTION_STABLEPET:
					if (getClass() != CLASS_DRUID) //Awakening TO DRUID
                        canTalk = false;
                    break;
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
	...
	_canSeeTransmog = fields[73].GetBool();	
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
		...
		stmt->setBool(index++, _canSeeTransmog);
    }
    else
    {
        // Update query
...
void Player::InitDataForForm(bool reapplyMods)
{
    ...
    switch (form)
    {
		...
		case FORM_CAT:
		{
			if (getPowerType() != POWER_MANA) //Awakening Cat to Energy now set to mana
		...
		case FORM_DIREBEAR:
		{
			if (getPowerType() != POWER_MANA)   //Awakening Bear to Rage now set to mana
				setPowerType(POWER_MANA);
			break;
		}
...
void Player::UpdateAreaDependentAuras(uint32 newArea)
{
    ...
    for (auto iter = m_ownedAuras.begin(); iter != m_ownedAuras.end();)
    {
...
Pet* Player::SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 duration, uint32 summonedBySpellId) // Awake
{
    Pet* pet = new Pet(this, petType);

	pet->SetSummonedBySpellId(summonedBySpellId); // Awake
