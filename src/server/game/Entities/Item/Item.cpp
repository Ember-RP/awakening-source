...
// Awake: Getting an Enchantment's Spell ID.
uint32 Item::GetEnchantmentSpellId(EnchantmentSlot slot) const
{
	uint32 spellid = 0;
	if (uint32 enchant_id = GetEnchantmentId(slot))
	{
		if (SpellItemEnchantmentEntry const* enchantEntry = sSpellItemEnchantmentStore.LookupEntry(enchant_id))
			if (enchantEntry->spellid[0])
				spellid = enchantEntry->spellid[0];
	}
	return spellid;
}
