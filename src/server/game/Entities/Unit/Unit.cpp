	...
	m_HostileRefManager(this), m_spellHistory(new SpellHistory(this))
{
	...
	// Awake
    for (uint8 i = 0; i < ARMOR_SUBCLASS_END; ++i)
        m_armorSubClass[i] = 0;
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
	...
	if (cleanDamage && damagetype == DIRECT_DAMAGE && this != victim && getPowerType() == POWER_MANA)
	{
	...
	if (!damage)
	{
		...
		if (cleanDamage && cleanDamage->absorbed_damage && victim->getPowerType() == POWER_MANA)
			victim->RewardRage(cleanDamage->absorbed_damage, 0, false);

		return 0;
	}

    // Rage from Damage made (only from direct weapon damage) This is for Bearform, as their standard is rage.
    ...
    if (!damage)
    {
        // Rage from absorbed damage This is for Bearform, as their standard is rage.
		//This doesn't appear to be working properly.
    ...
    else
    {
        ...
        if (this != victim && victim->getPowerType() == POWER_MANA)
        {
...
void Unit::_UpdateSpells(uint32 time)
{
    ...
    m_ownedAurasModified = false;

    for (auto i = m_ownedAuras.begin(); i != m_ownedAuras.end();)
    {
        if (i->second->isOwnerUpdated())
        {
            ++i;
            continue;
        }

        i->second->setOwnerUpdated(true);
        i->second->UpdateOwner(time, this);

        if (!m_ownedAurasModified)
        {
            ++i;
        }
        else
    ...
    for (auto i = m_ownedAuras.begin(); i != m_ownedAuras.end();)
    {
		i->second->setOwnerUpdated(false);
...
AuraApplication * Unit::_CreateAuraApplication(Aura* aura, uint8 effMask)
{
	...
	auto aurApp = new AuraApplication(this, caster, aura, effMask);
...
void Unit::_UnapplyAura(AuraApplication * aurApp, AuraRemoveMode removeMode)
{
    ...
    auto range = m_appliedAuras.equal_range(spellId);
    for (; range.first != range.second;)
    {
        if (range.first->second == aurApp)
        {
            _UnapplyAura(range.first, removeMode);
            return;
        }
        else
            ++range.first;
...
{
    for (auto itr = m_ownedAuras.lower_bound(spellId); itr != m_ownedAuras.upper_bound(spellId);)
...
void Unit::RemoveOwnedAura(Aura* aura, AuraRemoveMode removeMode)
{
	...
	auto const range = m_ownedAuras.equal_range(spellId);
...
{
    for (auto iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
        if (check(iter->second))
        {
            RemoveOwnedAura(iter);
...
void Unit::RemoveAllAuras()
{
    ...
    {
		for (auto aurAppIter = m_appliedAuras.begin(); aurAppIter != m_appliedAuras.end();)
            _UnapplyAura(aurAppIter, AURA_REMOVE_BY_DEFAULT);

        for (auto aurIter = m_ownedAuras.begin(); aurIter != m_ownedAuras.end();)
...
void Unit::GetDispellableAuraList(Unit* caster, uint32 dispelMask, DispelChargesList& dispelList, bool isReflect /*= false*/)  const
{
    ...
    auto const& auras = GetOwnedAuras();
...
{
	for (auto const &kvPair : GetAppliedAuras())
    {
        auto const aura = kvPair.second->GetBase();
        if (!kvPair.second->IsPositive() && aura->GetSpellInfo()->Attributes & flag && (!guid || aura->GetCasterGUID() == guid))
            return true;
    }
    return false;
}
...
bool Unit::HasAuraWithMechanic(uint32 mechanicMask) const
{
    for (auto const &kvPair : GetAppliedAuras())
    {
        auto const spellInfo = kvPair.second->GetBase()->GetSpellInfo();
        if (spellInfo->Mechanic && (mechanicMask & (1 << spellInfo->Mechanic)))
            return true;

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (kvPair.second->HasEffect(i) && spellInfo->Effects[i].Effect && spellInfo->Effects[i].Mechanic)
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 4993:
            {
				...
				for (auto const &kvPair : victim->GetAppliedAuras())
				{
                    Aura const* aura = kvPair.second->GetBase();
                    SpellInfo const* spell = aura->GetSpellInfo();
    ...
    if (typeMask & MELEE_BASED_TRIGGER_MASK && procTarget)
    {
        ...
        if (hitMask & (PROC_HIT_CRITICAL | PROC_HIT_PARRY | PROC_HIT_DODGE | PROC_HIT_BLOCK))
        {
            ...
            else // For attacker
            {
                ...
                if ((hitMask & PROC_HIT_DODGE) && GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_DRUID) //Awake
                {
					ModifyAuraState(AURA_STATE_OVERPOWER, true);
...
void Unit::ClearAllReactives()
{
    ...
    if (getClass() == CLASS_DRUID && HasAuraState(AURA_STATE_HUNTER_PARRY))
        ModifyAuraState(AURA_STATE_HUNTER_PARRY, false);
	if (getClass() == CLASS_DRUID && HasAuraState(AURA_STATE_OVERPOWER))
		ModifyAuraState(AURA_STATE_OVERPOWER, false);
}
        ...
        if (m_reactiveTimer[reactive] <= p_time)
        {
            ...
            switch (reactive)
            {
                ...
                case REACTIVE_OVERPOWER:
					// Awake
					if (getClass() == CLASS_DRUID && GetTypeId() == TYPEID_PLAYER)  //I don't think we want to modify this one AWAKENING
						ModifyAuraState(AURA_STATE_OVERPOWER, false); //ASCENSION change for Overpower, to remove dependency on combo points.
					break;
            }
        }
        else
