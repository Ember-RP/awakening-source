...
namespace LuaPlayer
{
	...
	/**
	*
	* Returns the [Spell] cost, of the specific [Player]
	*
	* @param uint32 spellid
	* @return uint32 powerCost
	*/

	int GetSpellCost(lua_State* L, Player* player)
	{
		uint32 spellId = Eluna::CHECKVAL<uint32>(L, 2);
		...
		if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
		{
			Eluna::Push(L, spellInfo->CalcPowerCost(player, spellInfo->GetSchoolMask()));
		}
		else
		{
			Eluna::Push(L, 0);
	...
	/**
	*
	*Returns the [Spell] powertype.
	*
	*@param uint32 spellid
	*@return uint32 powerType
	*/
	int GetSpellPowerType(lua_State* L, Player* player)
	{
		uint32 spellId = Eluna::CHECKVAL<uint32>(L, 2);
		...
		if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
		{
			Eluna::Push(L, spellInfo->PowerType);
		}
		else
		{
			Eluna::Push(L, 0);
	...
	/**
	*
	*Returns the [Spell] Cast time.
	*
	*@param uint32 spellid
	*@return uint32 casttime
	*/
	int GetSpellCastTime(lua_State* L, Player* player)
	{
		uint32 spellId = Eluna::CHECKVAL<uint32>(L, 2);
#ifdef TRINITY
		if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
		{
			...
			player->ModSpellCastTime(spellInfo, castTime);
			Eluna::Push(L, castTime);
		}
#endif
		return 1;
	}
	...
	*/
	int GetSpellRange(lua_State* L, Player* player)
	{
		uint32 spellId = Eluna::CHECKVAL<uint32>(L, 2);
#ifdef TRINITY
		if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
		{
			Eluna::Push(L, spellInfo->GetMaxRange(false, player));
		}
#endif TRINITY
		return 1;
	}
	...
	/**
	*
	* Returns the [Spell] cooldown, for the specific [Player]
	*
	* @param uint32 spellid
	* @return uint32 cooldown
	*/

	int GetSpellCooldown(lua_State* L, Player* player)
	{
		uint32 spellId = Eluna::CHECKVAL<uint32>(L, 2);
		...
		if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
		{
			TC_LOG_DEBUG("root", "Worked before spellcooldown assignment");
			uint32 spellCooldown = player->GetSpellHistory()->CalcCooldown(spellInfo);
			Eluna::Push(L, spellCooldown);
		}
#endif
		return 1;
	}
	...
	{
		uint32 spellId = Eluna::CHECKVAL<uint32>(L, 2);
		uint32 cooldownChange = Eluna::CHECKVAL<int32>(L, 3);
		player->GetSpellHistory()->ModifyCooldown(spellId, cooldownChange);
		return 0;
	}
	/*
	ModifyPower directly modifies the power of the player.
	This is in addition to the values associated by the DBC.	
	*/
	int ModifyPower(lua_State* L, Player* player)
	{
		int32 powerType = Eluna::CHECKVAL<int32>(L, 2);
		int32 costChange = Eluna::CHECKVAL<int32>(L, 3);
		Powers pow;
		switch (powerType)
		{
		case 0:
		...
		player->ModifyPower(pow, costChange);
		return 0;
	}
