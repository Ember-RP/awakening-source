...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
		...
		case SMART_ACTION_LOAD_TEMPLATE:
		{
			SmartAIEventList a = sSmartScriptMgr->GetScript(e.action.LoadTemplate.entry, mScriptType);
			if (a.empty())
				break;
			...
			if (me)
				FillScript(a, me, nullptr);
			else if (go)
				FillScript(a, go, nullptr);
			else if (trigger)
				FillScript(a, nullptr, trigger);
