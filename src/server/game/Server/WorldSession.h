...
class TC_GAME_API WorldSession
{
    public:
		...
		uint32 GetCurrentVendor() const { return m_currentVendorEntry; } // r00tz Multi-Vendor
		void SetCurrentVendor(uint32 vendorEntry) { m_currentVendorEntry = vendorEntry; } // r00tz Multi-Vendor
		...
		void SendListInventory(ObjectGuid guid, uint32 vendorEntry = 0); // r00tz Multi-Vendor
		...
		std::atomic<int32> m_menuTimeOut; //Awakening menu timeout

        void UpdateTimeOutTime(uint32 diff)
        {
			if (!_player && !m_inQueue)
			{
				m_menuTimeOut -= int32(diff);
			}
			else {
		...
		void ResetMenuTimeOutTime()
		{
			m_menuTimeOut = 600000;
		}
        ...
        bool IsConnectionIdle() const
        {
			if (!_player)
			{
				return m_menuTimeOut <= 0 && !m_inQueue;
			}
			else
			{
    ...
    private:
     ...
     // std::string m_LAddress;                             // Last Attempted Remote Adress - we can not set attempted ip for a non-existing session!
        ...
        uint32 expireTime;
        bool forceExit;
		uint32 m_currentVendorEntry; // r00tz Multi-Vendor
