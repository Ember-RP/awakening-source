...
WorldSocket::ReadDataHandlerResult WorldSocket::ReadDataHandler()
{
    ...
    switch (opcode)
    {
        ...
        default:
        {
			...
			_worldSession->ResetMenuTimeOutTime();
...
void WorldSocket::HandleAuthSessionCallback(std::shared_ptr<AuthSession> authSession, PreparedQueryResult result)
{
	...
	//ASCENSION: Insta-Log exploit helper
	if (sWorld->FindSession(account.Id))
	{
		SendAuthResponseError(AUTH_ALREADY_ONLINE);
		TC_LOG_ERROR("misc", "Attempted Login to existing session account ID");
		DelayedCloseSocket();
		return;
	}
