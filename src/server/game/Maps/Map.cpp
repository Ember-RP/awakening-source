...
void Map::ProcessRelocationNotifies(const uint32 diff)
{
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
		...
		float monsterSightRadius = sWorld->getFloatConfig(CONFIG_SIGHT_MONSTER);
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
				...
				Trinity::DelayedUnitRelocation cell_relocationCreature(cell, pair, *this, monsterSightRadius);
				Trinity::DelayedUnitRelocation cell_relocationPlayer(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
				TypeContainerVisitor<Trinity::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocationCreature);
				TypeContainerVisitor<Trinity::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocationPlayer);
...
void Map::RespawnCellAreaZoneCreature(uint32 cellZoneAreaId)
{
	...
	_cellAreaZoneLastRespawnedCreatureMap[cellZoneAreaId] = GameTime::GetGameTimeMS();
...
void Map::RespawnCellAreaZoneGameObject(uint32 cellZoneAreaId)
{
	...
	// Store this time, so we don't check so often
	_cellAreaZoneLastRespawnedGameObjectMap[cellZoneAreaId] = GameTime::GetGameTimeMS();
