...
void AnticheatCheckBase<type>::HackReport(Player* player, AnticheatData* playerData) const
{
    ...
    {
        uint32 actualTime = GameTime::GetGameTimeMS();
    ...
    if (!playerData->GetTotalReports())
        playerData->SetCreationTime(GameTime::GetGameTimeMS());
    ...
    uint32 diffTime = getMSTimeDiff(playerData->GetCreationTime(), GameTime::GetGameTimeMS()) / IN_MILLISECONDS;
    ...
    {
        playerData->SetLastReportTimer(GameTime::GetGameTimeMS());
