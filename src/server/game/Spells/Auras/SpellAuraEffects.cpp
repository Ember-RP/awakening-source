...
void AuraEffect::ApplySpellMod(Unit* target, bool apply)
{
    ...
    switch (GetMiscValue())
    {
        ...
        case SPELLMOD_EFFECT3:
        {
			...
			for (auto const &kvPair : target->GetAppliedAuras())
            {
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    else
    {
        ...
        auto &tAuras = target->GetAppliedAuras();
...
void AuraEffect::HandleAuraDummy(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // AT APPLY & REMOVE
	//AWAKENING: Selective Armor Contribs
	switch (GetId())
	{
		...
		case IMPROVED_BARKSKIN_2:
			switch (target->GetShapeshiftForm())
			{
				case FORM_NONE:
				case FORM_TRAVEL:
					target->ToPlayer()->_RecalculateBaseArmor();
					break;
				default:
					break;
			}
