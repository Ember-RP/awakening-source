...
m_procCooldown(std::chrono::steady_clock::time_point::min()), m_ownerUpdated()
{
...
void Aura::_ApplyForTarget(Unit* target, Unit* caster, AuraApplication * auraApp)
{
	...
	// r00tz
	m_applications.insert(ApplicationMap::value_type(target->GetGUID(), auraApp));
        ...
        if (addUnit)
        {
            ...
            else // (GetType() == UNIT_AURA_TYPE)
            {
                ...
                if (itr->first != GetOwner())
                {
                    ...
                    for (auto iter = itr->first->GetAppliedAuras().begin(); iter != itr->first->GetAppliedAuras().end(); ++iter)
                    {
    ...
    else
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_ROGUE:
				...
				// Awake Vanish change
				if ((GetId() == 1856 || 1857 || 26889) && !caster->HasSpell(1784))
